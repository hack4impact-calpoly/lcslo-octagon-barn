...
async function uploadDocument(file: File, user: UserResource | null | undefined, eventId: string, documentId: string) {
  try {
        ...
        {isUploaded && s3DocIdClient ? (
          ...
          <Button
            ...
            onClick={async () => {
              if (file) {
                try {
                  const s3Id = await uploadDocument(file, user, eventId as string, documentId as string);
